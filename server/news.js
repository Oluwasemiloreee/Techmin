...
(() => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Deep-link support: a URL like /news?q=MTN%20MoMo (e.g. from a
    // newsletter) pre-fills the search box so the reader lands directly on
    // that story instead of having to hunt for it on the general News page.
    const params = new URLSearchParams(location.search);
    const deepLinkQuery = params.get('q');
    if (deepLinkQuery) {
      state.query = deepLinkQuery;
      if (elements.search) elements.search.value = deepLinkQuery;
    }
